          ...
          <div className="flex gap-1 p-1 bg-black/40 border border-oro/10">
            {(['todos', 'basico', 'avanzado'] as const).map((t) => (
              <button
                ...
                className={`px-4 py-2 text-caption font-black uppercase tracking-widest transition-all ${activeType === t ? 'bg-oro/20 text-oro border border-oro/30' : 'text-oro/30 hover:text-oro'}`}
              >
            ...
            <tbody className="divide-y divide-oro/5 bg-black/40">
              {filtered.map((el) => (
                <tr key={el.id} className="hover:bg-oro/5 transition-colors group">
                  ...
                  {/* Tipo */}
                  <td className="py-5 px-8">
                    <span className={`px-3 py-1 text-caption font-black uppercase tracking-widest inline-block ninja-clip-xs ${el.tipo === 'basico' ? 'bg-naranja-naruto text-oro' : 'bg-oro/10 border border-oro/30 text-oro'}`}>
                  ...
                  <td className="py-5 px-8 text-right">
                    <div className="flex items-center justify-end gap-2.5">
                      <button
                        ...
                        className={`p-2.5 transition-all border ninja-clip-xs ${el.activo ? 'bg-oro/10 border-oro/20 text-oro hover:bg-oro hover:text-naranja-naruto' : 'bg-black/40 border-oro/5 text-oro/20 hover:border-oro/40 hover:text-oro'}`}
