...
export default function RasgosList({ initialRasgos }: RasgosListProps) {
  ...
  const toggleActive = async (id: number, currentStatus: boolean) => {
    setLoadingId(id);
    try {
      await AdminService.saveRasgo({ id, activo: !currentStatus });
      addToast(`Rasgo ${!currentStatus ? 'activado' : 'desactivado'}`, 'success');
      router.refresh();
    } catch (err: any) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col min-[1600px]:flex-row flex-wrap gap-6 justify-between items-stretch min-[1600px]:items-center bg-neutral-800/40 p-6 sm:p-10 xl:p-12 border border-oro/5 backdrop-blur-md relative overflow-hidden">
        ...
        {/* Filtro categoría + búsqueda + botón nuevo */}
          ...
          <table className="w-full text-left border-collapse min-w-[700px] table-fixed">
            <thead>
              <tr className="border-b border-oro/10 text-oro/70 text-caption xl:text-xs font-black uppercase tracking-[0.3em] bg-black/20">
                ...
                <th className="py-6 px-8 w-[15%]">Especial / Stat</th>
                <th className="py-6 px-8 text-right w-[15%]">Acciones</th>
              </tr>
            </thead>
              ...
              {filtered.map((r) => (
                <tr key={r.id} className="hover:bg-oro/5 transition-colors group">
                  ...
                  <td className="py-5 px-8">
                    <h3 className="text-base font-black text-oro uppercase italic tracking-tighter leading-none">{r.nombre}</h3>
                  </td>

                  {/* Categoría */}
                  <td className="py-5 px-8">
                    <span className="text-sm font-black text-oro/80 tracking-wider uppercase">{r.categoria}</span>
                  </td>
              ...
              {filtered.length === 0 && (
                <tr>
                  <td colSpan={5} className="py-24 text-center">
                    <p className="text-oro/10 font-black uppercase italic tracking-[0.3em] text-xs">Sin rasgos registrados</p>
                  </td>
                </tr>
              )}
            </tbody>
