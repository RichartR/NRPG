...
export default function FichaPublicPage() {
  ...
  const puntosGastados = Object.values(character.stats_base || {}).reduce((a, b) => Number(a) + Number(b), 0);
      ...
      <header className="sticky top-0 z-50 bg-zinc-950/80 border-b border-zinc-800/50 backdrop-blur-2xl">
        <div className="max-w-7xl mx-auto px-6 h-24 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            ...
            {isEditing && (
              <button onClick={() => save()} disabled={saving} className="flex items-center gap-3 px-8 py-4 bg-orange-600 hover:bg-orange-500 text-black rounded-[2rem] font-black text-xs uppercase tracking-widest transition-all shadow-xl shadow-orange-500/20 active:scale-95 disabled:opacity-50">
      ...
      <main className="max-w-7xl mx-auto p-6 md:p-10">
            ...
            <div className="lg:col-span-8 space-y-8">
              <SectionCard title="Información Básica" icon={User} color="emerald">
                 <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <DataField label="Nombre en Hobba" value={character.hobba_name} disabled={!isEditing} onChange={(v)=>updateField('hobba_name', v)} />
                    <DataField label="Tiempo en el RPG" value={character.tiempo_rpg} disabled={!isEditing} onChange={(v)=>updateField('tiempo_rpg', v)} />
                 </div>
              </SectionCard>

              <SectionCard title="Información de Personaje" icon={UserCircle} color="blue">
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <DataField label="Nombre Ninja" value={character.nombre_ninja} disabled={!isEditing} onChange={(v)=>updateField('nombre_ninja', v)} />
                    <SelectField 
                      label="Aldea" 
                      value={character.aldea_id} 
                      options={masters.aldeas.map(a=>({label:a.nombre_completo, value:a.id}))} 
                      disabled={!isEditing} 
                    ...
                    <DataField label="Rango Jerárquico" value={character.rango_jerarquico} disabled={!isEditing} onChange={(v)=>updateField('rango_jerarquico', v)} />
                 </div>
              </SectionCard>
              ...
              <SectionCard title="Ramas y Especialidades" icon={GitBranch} color="purple">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                   {[1, 2].map(slot => {
                     const pr = character.personajes_ramas?.find(r => r.slot === slot);
                     return (
                       <div key={slot} className="space-y-4 p-6 bg-zinc-900/50 border border-zinc-800 rounded-3xl">
                          ...
                          <SelectField label="Rama / Clan" value={pr?.rama_id} options={masters.ramas.map(r=>({label:r.nombre, value:r.id}))} disabled={!isEditing} onChange={(v)=>{
                            const newRamas = [...(character.personajes_ramas?.filter(r=>r.slot !== slot) || []), { slot, rama_id: Number(v), sub_especialidad_id: null, personaje_id: id }];
                            updateField('personajes_ramas', newRamas);
                          }} />
                          <SelectField label="Sub-Especialidad" value={pr?.sub_especialidad_id} options={masters.subEspecialidades.filter(s=>s.rama_id === pr?.rama_id).map(s=>({label:s.nombre, value:s.id}))} disabled={!isEditing} onChange={(v)=>{
                            const newRamas = [...(character.personajes_ramas?.filter(r=>r.slot !== slot) || []), { slot, rama_id: pr?.rama_id, sub_especialidad_id: Number(v), personaje_id: id }];
                            updateField('personajes_ramas', newRamas);
                          }} />
                       </div>
                     );
                    ...
                    return (
                      <div key={s} className="bg-zinc-900/50 border border-zinc-800/50 p-6 rounded-[2rem] group hover:border-orange-500/30 transition-all text-center">
                        ...
                        <input 
                          ...
                          value={val} 
                          disabled={!isEditing}
                          onChange={(e) => updateStat(s as keyof CharacterStats, parseInt(e.target.value))}
        ...
        {activeTab === 'inventario' && (
          <SectionCard title="Mochila y Equipo" icon={Briefcase} color="blue">
             <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {character.personajes_inventario?.map((pi) => (
                  <div key={pi.item_id} className="bg-zinc-900 p-6 border border-zinc-800 rounded-3xl flex justify-between items-center group">
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      {canEdit && <button onClick={()=>updateField('personajes_inventario', character.personajes_inventario?.filter(i=>i.item_id !== pi.item_id))} className="text-red-500 p-2 hover:bg-red-500/10 rounded-xl transition-all"><Trash2 className="w-4 h-4" /></button>}
                    </div>
                  </div>
                ))}
             </div>
             {canEdit && isEditing && (
               <div className="mt-12 pt-10 border-t border-zinc-800">
                  <SelectField label="Añadir Objeto al Inventario" options={masters.items.map(i=>({label:`${i.nombre} (${i.categoria})`, value:i.id}))} onChange={(v)=>{
                    const it = masters.items.find(i=>i.id === Number(v));
                    if (it) updateField('personajes_inventario', [...(character.personajes_inventario || []), { item_id: it.id, cantidad: 1, items_catalog: it, personaje_id: id }]);
                  }} />
               </div>
             )}
          </SectionCard>
        ...
        {activeTab === 'tecnicas' && (
          <SectionCard title="Artes Ninja" icon={Zap} color="orange">
             <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {character.personajes_tecnicas?.map((pt) => (
                  <div key={pt.tecnica_id} className="bg-zinc-900 p-6 border border-zinc-800 rounded-3xl flex justify-between items-center">
                    ...
                    {canEdit && <button onClick={()=>updateField('personajes_tecnicas', character.personajes_tecnicas?.filter(t=>t.tecnica_id !== pt.tecnica_id))} className="text-red-500 p-2 hover:bg-red-500/10 rounded-xl transition-all"><Trash2 className="w-4 h-4" /></button>}
                  </div>
                ))}
             </div>
             {canEdit && isEditing && (
               <div className="mt-12 pt-10 border-t border-zinc-800">
                  <SelectField label="Aprender Nueva Técnica" options={masters.tecnicas.map(t=>({label:`${t.nombre} (Rango ${t.rango})`, value:t.id}))} onChange={(v)=>{
                    const tec = masters.tecnicas.find(t=>t.id === Number(v));
                    if (tec) updateField('personajes_tecnicas', [...(character.personajes_tecnicas || []), { tecnica_id: tec.id, tecnicas_glosario: tec, personaje_id: id }]);
                  }} />
               </div>
             )}
          </SectionCard>
